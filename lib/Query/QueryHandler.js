...
class QueryHandler {
  ...
  static async getQueryDetails(queryCode) {
    const queryDetails = await sequelize.query(`SELECT TOP 1 * FROM tbl_Query WITH (NOLOCK) WHERE IsDeleted = 0 AND QueryCode = :queryCode`, {
      replacements: { queryCode },
    ...
    let parameterList = [...queryParameterID];
    if (parameterList.length < 1) {
      return { queryDetails: queryDetails[0], parameters: [] };
    }
    ...
    if (ValidationQuery) {
      ...
      if (validationResult.length > 0) {
        ...
        if (String(ValidationQueryParameterOperator) === "SOME") {
          ...
          const [firstItem] = validationResult;
          const firstValue = firstItem[Object.keys(firstItem)[0]];
          evaluationResult = firstValue === 1 || firstValue === "1";
        } else {
          const [firstItem] = validationResult;
          const firstValue = firstItem[Object.keys(firstItem)[0]];
  ...
  static async constructQuery(queryDetails, parameters, inputParams) {
    ...
    for (const param of filteredArray) {
      ...
      if (paramValue === undefined && IsOptional) {
        ...
        replacements[ParameterName] = replacedValue ?? paramValue ?? null;
      } else {
        ...
        // TO PREVENT NUMERIC RELATED VALUES AND LIKE OPERATORS TO BE REGEX VALIDATED
        if (
          ParameterType === "STRING" &&
          ValidationRegex &&
          ValidationRegex !== "LIKE" &&
          !new RegExp(ValidationRegex).test(paramValue)
        ) {
          throw new Error(ErrorMessage || `Invalid value for ${ParameterName}`);
        }
        ...
        replacements[ParameterName] =
          ValidationRegex && ValidationRegex === "LIKE"
            ? `%${replacements[ParameterName]}%`
            : replacements[ParameterName];
      }
    }
  ...
  static async executeQuery(queryCode, inputParams, req = null, t = null) {
    const { queryDetails, parameters } = await QueryHandler.getQueryDetails(queryCode);
    ...
    try {
      ...
      if (req != null) {
        results = await sequelize.query(queryText, {
          ...
          transaction: t,
          logging: (sql, timing) => {
            Log.LogAction(req, queryDetails.Name, sql);
          },
        });
      } else {
        results = await sequelize.query(queryText, {
