  ...
  async UploadProductImage(data, req) {
    try {
      ...
      if (!hasFolder) {
        ...
        let productImageName = "Image_" + data.imageIndex;
        let productImageDirectory =
          targetFolder + "/" + productImageName + (await this.getFileExtensionFromBase64(data.imageBase64));
        const bUploadProductImage = await this.uploadObject(productImageDirectory, data.imageBase64, req);
